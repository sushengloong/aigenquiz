...
async def fetch_url_and_generate_quizzes(job_id: uuid.UUID, input_url: str):
    print(f"Starting job {str(job_id)}")
...
#     """
    ...
    print(f"Finished job {str(job_id)}")

@app.post("/api/generate")
async def generate(generate_request: GenerateRequest, background_tasks: BackgroundTasks):
    job_uuid = uuid.uuid4()
    background_tasks.add_task(fetch_url_and_generate_quizzes, job_uuid, generate_request.url)
    ...
    async def event_stream():
        while True:
            for job_id, quizzes in list(job_output_quizzes.items()):
                ...
                print(f"Sending job {str(job_id)}")
                yield dict(data=quizzes_json)
                del job_output_quizzes[job_id]  # Clear update after sending
            await asyncio.sleep(1) # Polling interval

    return EventSourceResponse(event_stream(), media_type="text/event-stream", headers={'cache-control': 'no-transform'})
